...
local Raycaster = {
    ...
    draw_skybox = function(self, Player, Map)
        local skybox_tex = Map.skybox_texture
        local scale = 2
            ...
            if row_dist < Map.max_view_dist then
                for x = 0, screen_width - 1 do
                    ...
                    if cell_x > 0 and cell_x < Map.width
                        ...
                        local floor_tex = Map.layers.floors[cell_y][cell_x]
                        local ceiling_tex = Map.layers.ceilings[cell_y][cell_x]
    ...
    draw_walls = function(self, z_buffer, transparent_quads, Player, Map, texture_size, screen_width, screen_height)
        ...
        for x = 1, #z_buffer do
            ...
            while view_dist < Map.max_view_dist do
                ...
                if Map.layers.walls[Map_y][Map_x] > 0 then
            ...
            local wall_type = Map.layers.types[Map_y][Map_x]
            local wall_texture = Map.wall_textures[Map.layers.walls[Map_y][Map_x]]
    ...
    raycasting = function(self, Player, Map, screen_width, screen_height)
        local texture_size = Map.tile_width -- Size of textures but should be using texture.size idealy
